...
// Withdraw Requests Page Component
function WithdrawRequestsPage({ startDate, setStartDate, endDate, setEndDate }: any) {
  const { toast } = useToast();
  const [customerFilter, setCustomerFilter] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');

  const { data, isLoading, mutate } = useSWR(
    `/api/admin/withdrawals?customer=${customerFilter}&status=${statusFilter}&startDate=${startDate}&endDate=${endDate}`,
    fetcher,
    {
      revalidateOnFocus: false,
  ...
  const updateWithdrawalStatus = async (withdrawalId: string, status: string) => {
    try {
      ...
      const res = await fetch(`/api/admin/withdrawals/${withdrawalId}`, {
        ...
        body: JSON.stringify({ 
          status,
          notes: `Cập nhật trạng thái thành ${status} bởi quản trị viên` 
        })
      });
      ...
      mutate({
        ...
        withdrawals: withdrawals.map((w: any) => 
          w._id === withdrawalId ? { ...w, status } : w
        )
      }, false);
      
      toast({
        title: 'Thành công',
        description: result.message || `Đã cập nhật trạng thái yêu cầu rút tiền thành ${status}`,
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4 mb-4">
        ...
        <Button variant="outline" size="sm" className="w-full sm:w-auto text-white border-gray-600" onClick={() => {
          setCustomerFilter('');
          setStatusFilter('all');
          setStartDate('');
          setEndDate('');
              ...
              <TableBody>
                {withdrawals.map((withdrawal: any, index: number) => (
                  <TableRow key={index}>
                    ...
                    <TableCell className="text-white">{withdrawal.bankAccountNumber}</TableCell>
                        ...
                        {withdrawal.status === 'Chờ duyệt' ? (
                          <>
                            <Button 
                              ...
                              onClick={() => updateWithdrawalStatus(withdrawal._id, 'Đã duyệt')}
                              disabled={isUpdating[withdrawal._id]}
                            >
                              {isUpdating[withdrawal._id] ? (
                            ...
                            <Button 
                              size="sm" 
                              variant="outline" 
                              className="text-gray-600 bg-transparent border-gray-600 w-full sm:w-auto" 
                              onClick={() => updateWithdrawalStatus(withdrawal._id, 'Từ chối')}
                              disabled={isUpdating[withdrawal._id]}
                            >
                              {isUpdating[withdrawal._id] ? (
                                <Loader2 className="h-4 w-4 animate-spin" />
                          ...
                          <div className="text-sm text-gray-400">
                            {withdrawal.status === 'Đã duyệt' ? 'Đã phê duyệt' : 'Đã từ chối'}
                          </div>
                        )}
                      </div>
                    </TableCell>
  ...
  const renderActivePage = () => {
    ...
    switch (activePage) {
      ...
      case 'withdraw-requests':
        return <WithdrawRequestsPage {...commonProps} />;
      case 'order-history':
